...
void computeGraphsStats() {
    ...
    for (unsigned int i = 0; i < nGraphs; i++) {
        TString filename = "./lumigr_TEC_R5_W9_"+DETIDS[i]+".root";
        TString GraphName = "lumigr_"+DETIDS[i];
    ...
    gMax->GetYaxis()->SetRangeUser(0, 400); // Ajuster l'axe Y
    gMax->GetXaxis()->SetTitle("Int. Lumi [fb^{-1}] "); // Ajuster l'axe X
    gMax->GetYaxis()->SetTitle("V_{fd} [V] "); // Ajuster l'axe X
   ...
   TLine *line = new TLine(391,0,391,400);
    line->SetLineWidth(6);
   line->SetLineStyle(2);
